...
fastify.get("/api/githubrepos/", async (_, serverReply) => {
  const cachedResult = IS_DEBUG
    ? undefined
    : memoryCacheClient.get(PAYLOAD_CACHE_KEY);
...
fastify.get("/rpc/redirect/", async (request, serverReply) => {
  if (request.query.where) {
    serverReply.code(301).redirect(request.query.where);
  } else {
